...
def aorc_as_rate(dataFrame: pd.DataFrame):
    ...
    try:
        partial_calc__TMP_2maboveground_apply_rho = dataFrame[
            "TMP_2maboveground"
        ].apply(rho)
    except KeyError as e:
        print(
            f"KeyError: TMP_2maboveground not found in dataFrame. Columns: {dataFrame.columns}"
        )
        raise e
    try:
        ...
        print(
            f"KeyError: APCP_surface not found in dataFrame. Columns: {dataFrame.columns}"
        )
        raise e
    partial_calc__shifted_div_rho = (
        partial_calc__APCP_surface_shifted / partial_calc__TMP_2maboveground_apply_rho
    )
...
for filename, df in output_data.items():
    ...
    try:
        ...
        shifted_diff[filename] = df["APCP_surface_shifted"].shift(-1) * 1000 - df[
            "APCP_surface"
        ].shift(1)
    except KeyError as e:
...
def get_allocated_space(
    aspect_ratio, group_count, width_grid=2, height_grid=1, resolution_mult=4
):
    ...
    print(
        f"adjusted_width: {adjusted_width}, width_per_group: {width_per_group}, width_per_group_2: {width_per_group_2}"
    )
...
def matplotlib_cmap_to_pil(
    cmap: mcolors.Colormap, index, min_val=0, max_val=1
) -> tuple:
...
def necessary_bounds_for_padding(
    image_bbox, new_rect, padding, threshold_ratio=0.5
) -> tuple[bool, tuple]:
    """
    ...
    threshold_padding = [
        int(-x * threshold_ratio) if i > 1 else 0 for i, x in enumerate(padding)
    ]
    ...
    for i, group_alloc in list(group_allocs.items()):
        ...
        def datapoint_bbox(index, slot_bbox=slot_bbox(0)):
            ...
            x = (
                accessible_bbox[0]
                if index == 0
                else accessible_bbox[0] + total_width - datapoint_dims[0]
            )
            y = accessible_bbox[1]
        ...
        def get_subcomponent_bboxes():
            ...
            all_bboxes["columns"] = [
                column_bbox(_k, group_bbox=all_bboxes["group"])
                for _k in range(num_cols)
            ]
            all_bboxes["slots"] = [
                [
                    slot_bbox(j, column_bbox=all_bboxes["columns"][_k])
...
cmp_img = series_comparisons_image(
    adjusted_width, height, group_alloc, apcp_shifted, apcp, cmap_heatmap
)
